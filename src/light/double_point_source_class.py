...
class DoublePointSourceClass(light_class.LightSourceClass):
    def __init__(self, p0=np.array([0, 0]), n0=np.array([1, 0]), spacing=1*mm, fan_angle=30*deg, wavelength=660*nm, intensity=1, intensity_distribution='equiangular', plot_color=None):
    ...
    def generate_rays(self, N_rays):
        ...
        for ind in range(len(d_angle_range)):
            ...
            ray = light_class.RayClass(p0=self.p0_source[ind_source], r=r, intensity=intensity, wavelength=self.wavelength, N=N_air, ray_parent=None, source_element=self, lightsource_ID=self.ID, plot_color=col)
            self.add_ray(ray)
