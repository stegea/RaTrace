...
class LaserClass(light_class.LightSourceClass):
    def __init__(self, p0=np.array([0,0]), n0=np.array([0,-1]), beamwaist=100*µm, beamwaist_distance=100*mm, sampling_width_at_BW=300*µm, wavelength=660*nm, M2=1, intensity=1, plot_color='wavelength'):
    ...
    def generate_rays(self, N_rays):
        ...
        for i_ray in range(N_rays):
            ...
            ray = light_class.RayClass(p0=self.p0, r=r, intensity=None, wavelength=self.wavelength, N=N_air, ray_parent=None, source_element=self, lightsource_ID=self.ID, plot_color=self.plot_color)
            self.add_ray(ray)
