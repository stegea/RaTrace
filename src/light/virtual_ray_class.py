...
class VirtualRayClass(light_class.LightSourceClass):
    def __init__(self, origin=None, destination=None, target_p0=None, target_n0=None, wavelength=660*nm, intensity=1, plot_color=None):
        self.origin = origin
    ...
    def generate_ray(self, origin_element):
        ...
        if isinstance(origin_element, imager_class.ImagerClass)  and  ('centroid' in self.origin)  and  (self.destination is not None):
            ...
            ray = light_class.RayClass(p0=self.p0, r=self.n0, intensity=self.intensity, wavelength=self.wavelength, N=N_air, ray_parent=None, source_element=self, lightsource_ID=self.ID, plot_color = self.plot_color, is_virtual=True)
