...
class ElementClass:
    ...
    def __init__(self, p0, n0, pts, is_active=True, is_visible=True):
        self.p0 = np.array(p0)    # Some principal center point on the first surface
        self.n0 = geometry.normalize(np.array(n0))    # Principal normal orientation, connected to p0
        self.pts = np.array(pts)  # The points describing the surface, could be for raytracing purposes, could be for plotting, or both. Points are described in CLOCKWISE order.
