    ...
    def plot(self, graph):
        if self.is_visible:
            if config.getboolean('view', 'plot_elements_in_BW'):
                ...
                edgecolor = 'cyan'
            graph.plot([self.pts[0][X], self.pts[1][X]], [self.pts[0][Y], self.pts[1][Y]], color=edgecolor, linewidth=3, linestyle='solid', alpha=1, zorder=5)
            varia.plot_arrow_end_at_P(graph, P=self.pts[0], r= self.r[0], s=np.sign(self.f)*self.diameter/50, angle=30*deg, col=edgecolor)
            varia.plot_arrow_end_at_P(graph, P=self.pts[1], r=-self.r[0], s=np.sign(self.f)*self.diameter/50, angle=30*deg, col=edgecolor)
